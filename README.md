...
## Path to actually selling this

Since the plan is to sell this one separately: personal use needs nothing beyond "Load unpacked." Selling it needs a Chrome Web Store listing — submitted as a plain `.zip` of this folder through the Developer Dashboard (one-time $5 registration fee), not via the "Pack extension" button in `chrome://extensions` (that produces a `.crx` for direct/sideloaded distribution, a different path). Review currently takes anywhere from under an hour to a few weeks depending on Google's queue.

Icons are done (`icons/`, referenced from `manifest.json`) — regenerate anytime with `python3 icons/generate_icons.py` (requires Pillow). The license-gating mechanism is built and working, pending the ExtensionPay placeholder above. Still outstanding before a real listing: registering the extension and connecting Stripe at extensionpay.com (do this yourself), a privacy policy URL, a support contact, and at least one store screenshot.
